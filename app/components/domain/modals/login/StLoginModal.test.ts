...
import StLoginModal from './StLoginModal.vue';

describe('StLoginModal', () => {
  it('não renderiza conteúdo quando open=false', () => {
    const wrapper = mount(StLoginModal, {
      props: { open: false }
    });
  ...
  it('renderiza quando open=true e fecha ao clicar em Entrar', async () => {
    const wrapper = mount(StLoginModal, {
      props: { open: true }
    });
