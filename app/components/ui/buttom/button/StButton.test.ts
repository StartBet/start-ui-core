...
describe('StButton', () => {
  ...
  it('aplica variant outline + color secondary', () => {
    const wrapper = mount(StButton, { props: { variant: 'outline', color: 'secondary' } })
    const list = classList(wrapper.attributes('class'))
    expect(list).toContain('bg-transparent')
    expect(list).toContain('border-content-secondary')
    expect(list).toContain('text-content-secondary')
  })

  it('aplica size small', () => {
    const wrapper = mount(StButton, { props: { size: 'small' }, slots: { default: 'Ok' } })
    const list = classList(wrapper.attributes('class'))
  ...
  it('aplica fullWidth', () => {
    const wrapper = mount(StButton, { props: { fullWidth: true }, slots: { default: 'Continuar' } })
    const list = classList(wrapper.attributes('class'))
