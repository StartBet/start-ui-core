...
describe('StPaper', () => {
  it('renderiza o slot e aplica defaults', () => {
    const wrapper = mount(StPaper, { slots: { default: '<div data-testid="x" />' } })
  ...
  it('aplica variant, border e elevation', () => {
    const wrapper = mount(StPaper, {
      props: { variant: 'surface-warning', border: '2', elevation: 3, borderRadius: '2' }
    })
    const cls = wrapper.attributes('class')
  ...
  it('aplica interactive com hover e active', () => {
    const wrapper = mount(StPaper, { props: { interactive: true, elevation: 2 } })
    const cls = wrapper.attributes('class')
