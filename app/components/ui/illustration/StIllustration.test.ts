...
const { loader } = vi.hoisted(() => ({
  loader: vi.fn(async () => '<svg xmlns="http://www.w3.org/2000/svg"><rect /></svg>')
}))
...
describe('StIllustration', () => {
  ...
  it('renderiza img quando carrega e aplica alt/width/height e $attrs', async () => {
    const wrapper = mount(StIllustration, {
      props: { name: 'brand/brand-dark', alt: 'Brand', width: 56, height: '56', className: 'x' },
      attrs: { class: 'y', style: 'color: red;', loading: 'lazy', decoding: 'async' }
    })
  ...
  it('usa cache e não recarrega o mesmo name', async () => {
    const a = mount(StIllustration, { props: { name: 'brand/brand-dark', alt: 'A' } })
    await flushPromises()
    await flushPromises()
    a.unmount()

    const b = mount(StIllustration, { props: { name: 'brand/brand-dark', alt: 'B' } })
    await flushPromises()
    await flushPromises()
    b.unmount()
  ...
  it('não renderiza quando o name não existe no registry', async () => {
    const wrapper = mount(StIllustration, {
      props: { name: 'does-not-exist' as any, alt: 'X' }
    })
