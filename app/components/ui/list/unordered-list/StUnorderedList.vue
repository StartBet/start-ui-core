...
import type { StListContextValue, StUnorderedListProps } from '~/components/ui/list/StList.interface'
import { buildUnorderedListClasses, stListContextKey } from '~/components/ui/list/styleStList'

defineOptions({ name: 'StUnorderedList', inheritAttrs: false })

const props = withDefaults(defineProps<StUnorderedListProps>(), {
  dense: false,
...
const classes = computed(() => buildUnorderedListClasses(props, renderOrientation.value))
...
<template>
  <ul :class="wrapperClass" :style="wrapperStyle" v-bind="listAttrs">
    <slot />
  </ul>
</template>
